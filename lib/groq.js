...
import { analyzeRepoStatic } from './analyzers/static.js';
import { detectSlop } from './analyzers/slop.js';

const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });

// ─── Prompt Engineering ───────────────────────────────────────────────────────

function getTechStackDirectives(fileContents) {
  ...
  for (const f of fileContents) {
    ...
    if (contentLower.includes('mongoose') || contentLower.includes('mongodb')) hasMongoose = true;
    if (contentLower.includes('pg') || contentLower.includes('postgres')) hasPostgres = true;
    if (contentLower.includes('sqlite')) hasSqlite = true;
    if (contentLower.includes('mysql')) hasMysql = true;
  }

  let directives = '\n\n# TECHNOLOGY STACK DIRECTIVES:';
  if (hasMongoose && !hasPostgres && !hasSqlite && !hasMysql) {
    directives += '\n- This project uses MongoDB/Mongoose (NoSQL). Do NOT flag or report SQL injection vulnerabilities. Only report NoSQL or query-level injection if user input is unescaped in dynamic database operators.';
  } else if (!hasMongoose && (hasPostgres || hasSqlite || hasMysql)) {
    directives += '\n- This project uses SQL. Verify SQL injection patterns where raw queries are concatenated.';
  } else {
    directives += '\n- Pay close attention to what databases are listed in package.json/dependencies before assuming any SQL or NoSQL vulnerability type.';
  }

  directives += `
- DECLARATION VS. EXECUTION CONSTRAINT: Do not flag Schema definitions, configuration declarations, types, or models (e.g. Mongoose Schemas, Prisma schemas, TypeScript interfaces) for active runtime vulnerabilities like query injection, missing error handlers, or unhandled exceptions. Limit such findings to routers, controllers, services, and execution paths.
- NO BOILERPLATE FINDINGS: Never flag generic warnings like 'use dependency injection', 'minimize coupling', or 'remove unused imports' unless they actively cause memory leaks, build failures, or extreme performance degradation. Every finding must be highly specific to the code snippet provided.
- IMPORT CAUTION: Do NOT flag imports from standard libraries or external npm packages (like 'firebase-admin/app', 'groq-sdk', 'firebase/app') as 'Circular Dependency' or 'Self-Import Risk' simply because the current file shares a name with the package. Only flag true recursive self-imports involving local relative files (e.g. './module' importing itself).
`;

  return directives;
}

function buildAuditPrompt(repoName, fileTree, codeChunks, cveFindings = [], techStackDirectives = '') {
...
${confirmedSection}
INSTRUCTIONS:
- Analyze the code above for security vulnerabilities, architectural issues, performance problems, test coverage gaps, documentation quality, and general code smells.
...
Finding types: "security" | "architecture" | "performance" | "test_coverage" | "documentation" | "code_smell"
...
export async function runAudit(repoName, fileTree, fileContents) {
  ...
  // 1. Run local analyzers
  const staticResult = analyzeRepoStatic(fileContents);
  const slopResult = detectSlop(fileContents);

  const allFindings = [...staticResult.findings, ...slopResult.findings];
  let aggregatedScores = { security: [], architecture: [], performance: [], test_coverage: [], documentation: [] };
  ...
  // Cap at 2 chunks to avoid serverless timeouts on Vercel (max 60s)
  const chunksToProcess = chunks.slice(0, 2);

  const sleep = (ms) => new Promise(r => setTimeout(r, ms));
  const techStackDirectives = getTechStackDirectives(fileContents);

  const cveFindings = await checkCVEs(fileContents);

  const chunkPromises = chunksToProcess.map(async (chunk, index) => {
    const prompt = buildAuditPrompt(repoName, fileTree, chunk, cveFindings, techStackDirectives);
    let attempts = 0;
    const maxAttempts = 4;
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (err?.error?.code === 'rate_limit_exceeded' || err.message.toLowerCase().includes('rate limit') || err.message.includes('429')) {
          attempts++;
          console.warn(`Rate limit hit in audit chunk ${index}. Waiting 30s before retry ${attempts}/${maxAttempts}...`);
          await sleep(30000 + (index * 5000));
        } else {
  ...
  results.forEach(r => {
    if (r.status === 'fulfilled' && r.value) {
      ...
      if (result.scores) {
        ...
        if (result.scores.test_coverage) aggregatedScores.test_coverage.push(result.scores.test_coverage);
        if (result.scores.documentation) aggregatedScores.documentation.push(result.scores.documentation);
      }
  ...
  let devopsScore = 45;
  const hasDocker = fileTree.some(f => f.path.toLowerCase().includes('dockerfile'));
  const hasCI = fileTree.some(f => f.path.toLowerCase().includes('.github/workflows'));
  if (hasDocker) devopsScore += 25;
  if (hasCI) devopsScore += 25;

  // Calculate Health Score
  const healthScore = Math.max(50, Math.min(100, 100 - (staticResult.findings.length * 5)));

  // Aggregate LLM-based scores
  const baseSecurity = avg(aggregatedScores.security) || 75;
  const baseArchitecture = avg(aggregatedScores.architecture) || 75;
  const basePerformance = avg(aggregatedScores.performance) || 75;
  const baseTestCoverage = avg(aggregatedScores.test_coverage) || 50;
  const baseDocumentation = avg(aggregatedScores.documentation) || 50;

  const scores = {
    security: baseSecurity,
    architecture: Math.round((baseArchitecture * 0.6) + (staticResult.scores.architecture * 0.4)),
    performance: basePerformance,
    slop: slopResult.slopScore,
    test_coverage: baseTestCoverage,
    documentation: baseDocumentation,
    health: healthScore,
  };

  // Overall score now correctly includes all 6 dimensions
  ...
  return {
    findings: deduped.slice(0, 50), // Max 50 findings
    scores,
    summary,
  };
}
...
function sanitizeCustomPrompt(prompt) {
  ...
  const techStackDirectives = getTechStackDirectives(fileContents);
  basePrompt += techStackDirectives;

  const fileList = filteredTree.map((f) => f.path).join('\n');
  const allChunks = chunkFiles(filteredContents, 25000); 
  const chunks = allChunks.slice(0, 2); // Cap at 2 chunks to avoid Vercel 60s timeout
  ...
  let aggregatedData = {
    findings: [...staticResult.findings, ...slopResult.findings],
    strengths: [],
    scores: [],
    summary: null,
    priority: null,
  ...
  const finalScores = { overall: 75, security: 75, architecture: 75, performance: 75, slop: 75, devops: 75, readiness: 75, health: 75 };
  ...
  // Calculate fallbacks and dependencies
  finalScores.slop = slopResult.slopScore || 75;
  ...
  if (hasDocker) devopsScore += 25;
  if (hasCI) devopsScore += 25;
  finalScores.devops = devopsScore;

  finalScores.readiness = Math.round(((finalScores.security || 75) * 0.35) + ((finalScores.performance || 75) * 0.35) + (devopsScore * 0.3));
  finalScores.health = Math.max(50, Math.min(100, 100 - (staticResult.findings.length * 5)));

  finalScores.overall = Math.round(
    (finalScores.security * 0.2) +
    (finalScores.architecture * 0.15) +
    (finalScores.performance * 0.15) +
    (finalScores.slop * 0.15) +
    (finalScores.devops * 0.1) +
    (finalScores.readiness * 0.15) +
    (finalScores.health * 0.1)
  );
