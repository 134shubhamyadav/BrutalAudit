...
export async function runAudit(repoName, fileTree, fileContents) {
  ...
  // Cap at 4 chunks to avoid massive parallel requests and serverless timeouts
  ...
  const scores = {
    ...
    devops: devopsScore,
    readiness: Math.round((baseSecurity * 0.35) + (basePerformance * 0.35) + (devopsScore * 0.3)),
    health: healthScore
  };
...
function sanitizeCustomPrompt(prompt) {
  ...
  const chunks = allChunks.slice(0, 4); // Cap at 4 chunks for detailed audit as well
